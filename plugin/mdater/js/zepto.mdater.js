(function($){
	$.fn.mdater = function(config){
		...
		var F = {
			...
			//获取当天零点的时间戳
			getCurrentDateZeroTime : function(){
				var currentDate = new Date();
					currentZeroMonth = (currentDate.getMonth() + 1) < 10 ? currentZeroMonth = '0' + (currentDate.getMonth() + 1) : currentZeroMonth = (currentDate.getMonth() + 1),
					currentZeroDate = currentDate.getDate() < 10 ? currentZeroDate = '0' + currentDate.getDate() : currentZeroDate = currentDate.getDate();
				return new Date((1900 + currentDate.getYear()) + '-' + currentZeroMonth + '-' + currentZeroDate).getTime()/1000 - 8*3600;
			}
		}
		...
		var mdater = {
			...
			saveCheckedDate : function(){
				if(this.value.date){
					...
					var withZeroMonth = (this.value.month + 1) < 10 ? withZeroMonth = '0' + (this.value.month + 1) : withZeroMonth = (this.value.month + 1);
					var withZeroDate = this.value.date < 10 ? withZeroDate = '0' + this.value.date : withZeroDate = this.value.date;
					
					this.selectDate = this.value.year + '-' + withZeroMonth + '-' + withZeroDate;
				}
			},
			//将上一次保存的数据恢复到界面
			...
			afterShowPanel : function (mask,panel) {
				...
				panel.delegates({
					'.change_month' : function(){
						...
						option.changeMonth && option.changeMonth();
					},
					'.change_year' : function(){
						var add = $(this).hasClass('md_next') ? 1 : -1;
						_this._changeYear(add);
						
						option.changeYear && option.changeYear();
					},
					'.out_left, .out_right' : {
					...
					'.md_datearea li' : function(){
						...
						mdater.value.date = $this.text();

						_this.saveCheckedDate();
						
					/*	console.log('mdater.value.year',mdater.value.year);
						console.log('mdater.value.month',mdater.value.month);
						console.log('mdater.value.date',mdater.value.date);

*/
						//判断是否满足当前的条件
						//星期5 16:00前都可以选，!只能选下周二及以后
						var _newDate = new Date(),
							_newDateOfWeek = _newDate.getDay();// 当前日期星期几
							_newDateTime = parseInt(_newDate.getTime()/1000);//当前世间戳
							_newDateAtSixteenTime = parseInt(new Date(mdater.value.lastCheckedDate + ' 16:00:00').getTime()/1000),//当天16点的时间戳
							_newDateOfZeroTime = new Date(_this.selectDate).getTime()/1000 - 8*3600;

						//当天是星期六、天 -->> 不能选
						if (_newDateOfWeek == 5 && _newDateOfZeroTime - F.getCurrentDateZeroTime() <= 3600*24*3) {//星期五
							return;
						}else if(_newDateOfWeek == 6 && _newDateOfZeroTime - F.getCurrentDateZeroTime() <= 3600*24*2){//星期六
							return;
						}else if(_newDateOfWeek == 0 && _newDateOfZeroTime - F.getCurrentDateZeroTime() <= 3600*24*1){//星期天
							return;
						}
/*
						console.log('当前选中日期',_this.selectDate)
						console.log('当天0点的时间戳',F.getCurrentDateZeroTime())
						console.log('当前选中的日期零点的时间戳',_newDateOfZeroTime) */
						...
						option.onSelect && option.onSelect($this.text(),_this.selectDate);
