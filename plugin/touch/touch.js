...
(function(win) {
	...
	var touchMain = {
        ...
        touchMove2: function(event,leftcallback,rightcallback){
            var self = this;
            try{
                var touch = event.touches[0], //获取第一个触点
                    x = Number(touch.pageX), //页面触点X坐标
                ...
                //判断滑动方向
                if (self.options.startX - x > 15 && self.options.startY - y < 10) {
                    //alert('左滑了！');
                ...
                console.log('x',x)
                console.log('self.options.startX',self.options.startX)

                console.log('y',y)
                console.log('self.options.startY',self.options.startY)
