; (function (win) {
	...
	var HSKJ = {
		...
		renderTable: function (opts) {
			...
			layui.table.render({
				...
				, done: function (res, curr, count) {
					console.log('渲染表格返回数据：', res);
					console.log('渲染表格返回数据2：', res.code);
					if (res.code == '100012'){
						HSKJ.toLoginPage();
						return;
					}
		...
		renderCommonDetail: function(data, dataForChinese, callback){
			console.log('renderCommonDetail', data)
			var html = [];
		 ...
		 * @return {[type]}
		 */
		getSystemparameter: function(keyword, callback){
			HSKJ.GET({
				url: 'system/systemparameter/query',
				data: {
					keyword: keyword
				},
				beforeSend: function () {
		...
		getAddress: function(address, callback){
			$.ajax({
				dataType:'jsonp',  
				url: 'https://api.map.baidu.com/geocoder/v2/?address='+ address +'&output=json&ak='+ ENV.AMAP_KEY,
				success: function(json){
