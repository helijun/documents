	...
	<div class="table">
		...
		<div class="container"  v-loading="is_loading" element-loading-text="资源加载中，请稍候..." element-loading-spinner="el-icon-loading">
			...
			<el-table v-if="!is_loading" :data="tableData" border style="width: 100%" ref="multipleTable">
				<el-table-column prop="checkOrgName" label="采集网点"/>
				<el-table-column prop="sendOrgName" label="检测网点"/>
				<el-table-column prop="applyName" label="姓名"/>
				<el-table-column prop="sex" label="性别"/>
				<el-table-column prop="address" label="地址"/>
				<el-table-column prop="sendTime" label="送查时间"/>
		...
		<el-dialog title="送样" :visible.sync="editVisible" width="400px">
			<el-form ref="editForm" :model="editForm" :rules="rules" label-width="117px">
				<el-form-item label="此次送样数量" prop="applyNumber">
					<el-input v-model="total" class="handle-input"></el-input>
				</el-form-item>
				<el-form-item label="送样机构" prop="checkOrgNumber">
	    ...
	    handleData2(action,_data) {
		 ...
		 getData() {
			...
			var parm = {
					 ...
					 sstartTime: moment(this.recordDate[0]).format("YYYY-MM-DD HH:mm:ss"),
					 sendTime: moment(this.recordDate[1]).format("YYYY-MM-DD HH:mm:ss"),
