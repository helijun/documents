	...
	<div class="table">
		...
		<div class="container"  v-loading="is_loading" element-loading-text="资源加载中，请稍候..." element-loading-spinner="el-icon-loading">
				...
				<el-row type="flex" justify="space-between" align="center">
					<el-col>
						<div class="select-tip">所属县区</div>
							<el-select v-model="orgArea" placeholder="所属县区" :change="getcountryTown"  @change="getcountryTown" >
								<el-option v-for="(item, i) in countryData" :key="i" :label="item.name" :value="item.code"></el-option>
							</el-select>

							<div class="select-tip">所属镇街</div>
							<el-select v-model="town" placeholder="所属县区" :change="select_stauts"  @change="select_stauts" >
					...
					<el-button type="primary" class="admin-btn" @click="handleAdd">增加采集网点</el-button>
				</el-row>
			</div>
			<!-- 开始查询条件 -->
			<!-- 开始数据列表 -->
			<el-table v-if="!is_loading" :data="tableData" border style="width: 100%" ref="multipleTable">
				...
				<el-table-column prop="townName" label="所属镇街"/>
		...
		<!-- 结束列表 -->
		<!-- 开始增加弹出框 -->
		<el-dialog title="增加采集网点管理" :visible.sync="addVisible" width="400px">
			<el-form ref="addForm" :model="addForm" :rules="rules" label-width="117px">
				...
				<el-form-item label="所属县区" prop="orgArea" >
				<el-select v-model="addForm.orgArea" placeholder="所属县区" :change="getcountryTown2"  @change="getcountryTown2"  class="handle-input">
					<el-option v-for="(item, i) in countryData" :key="i" :label="item.name" :value="item.code"></el-option>
				</el-select>
				</el-form-item>
		...
		<!-- 结束增加弹出框 -->
		<!-- 开始修改弹出框 -->
		<el-dialog title="修改采集网点管理" :visible.sync="editVisible" width="400px">
			<el-form ref="editForm" :model="editForm" :rules="rules" label-width="117px">
				...
				<el-form-item label="所属县区" prop="orgArea" class="handle-input">
				<el-select v-model="editForm.orgArea" placeholder="所属县区" :change="getcountryTown3"  @change="getcountryTown3">
					<el-option v-for="(item, i) in countryData" :key="i" :label="item.name" :value="item.code"></el-option>
				</el-select>
				</el-form-item>
				<el-form-item label="所属镇街" prop="town">
					<el-select v-model="editForm.town" placeholder="所属镇街" :change="select_stauts"  @change="select_stauts">
					<el-option v-for="(item, i) in townData" :key="i" :label="item.text" :value="item.value"></el-option>
				</el-select>
				</el-form-item>
				<el-form-item label="负责人" prop="principal">
	 ...
	 methods: {
		...
		getcountryTown() {
			axios.post({url: api.commn.action,
				...
				getcountryTown2() {
			axios.post({url: api.commn.action,
				 data: {model:'citybyparent',action:'select',code:this.addForm.orgArea}
			 }).then(res => {
				 if (res.code == 0) {
					 this.townData = res.data;
				 } else {
					 this.$message.error(res.message);
						...
						getcountryTown3() {
			axios.post({url: api.commn.action,
				 data: {model:'citybyparent',action:'select',code:this.editForm.orgArea}
			 }).then(res => {
				 if (res.code == 0) {
					 this.townData = res.data;
				 } else {
