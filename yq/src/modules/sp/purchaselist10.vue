	...
	<div class="table">
		...
		<div class="container"  v-loading="is_loading" element-loading-text="资源加载中，请稍候..." element-loading-spinner="el-icon-loading">
			...
			<el-table v-if="!is_loading" :data="tableData" border style="width: 100%" ref="multipleTable">
				...
				<el-table-column label="备案表">
					<template slot-scope="scope"  fixed="right">
						<span v-if="scope.row.recordFile1 != ''" @click="downloadfile2(scope.$index, scope.row)" style="text-decoration:underline;cursor:hand；color:red">附件下载</span>               
					</template>
                </el-table-column>
				<el-table-column prop="statename" label="状态"/>
				<el-table-column label="操作" width="180" fixed="right">
				<template slot-scope="scope">
					<el-button size="small" @click="handleEdit(scope.$index, scope.row)">完结订单</el-button>
				</template>
				</el-table-column>
...
export default {
	 ...
	 methods: {
		 ...
		 handleEdit(index, row) {
			...
			var param={
                 action:"update",
				 model:"tb_purchase_record_state6",
			 ...
			 axios.post({url: api.commn.action,
				 ...
				 if (res.code == 0) {
					this.getData();
					this.sendWXMsg(this.editForm.applyNumber,6,this.editForm.orgNumber);
                    alert('订单已完结');
				 } else {
		 ...
		 downloadfile2(index, row)
		 {
			if(row.recordFile1 != '')
				{
					window.open(row.recordFile1, "_blank");
				}			 
		 }
	 },
};
