<template>
	<div class="table">
		...
		<div class="container"  v-loading="is_loading" element-loading-text="资源加载中，请稍候..." element-loading-spinner="el-icon-loading">
			...
			<el-table v-if="!is_loading" :data="tableData" border style="width: 100%" ref="multipleTable">
				...
				<template slot-scope="scope">
					<el-button size="small" type="text" @click="handleEdit(scope.$index, scope.row)">打印备案单</el-button>
					<el-button size="small" type="text" @click="handleEdit2(scope.$index, scope.row)">打印用章单</el-button>
                    <el-button size="small" type="text" @click="handleEdit3(scope.$index, scope.row)">上传盖章单</el-button>
				</template>
				</el-table-column>
    	...
    	<!-- 开始增加弹出框 -->
		<el-dialog title="审核通过" :visible.sync="editVisible3" width="400px">
			<el-form label-width="117px">
				<el-form-item label="上会时间" prop="applyNumber">
                        ...
                        :on-success="handleUploadSuccess"
						            list-type="picture"
                        :limit="1"> <el-button size="small" type="primary">选择文件</el-button>
                    </el-upload>
				</el-form-item>
                <el-form-item label="上会时间" prop="applyNumber">
					<el-upload
			...
			<span slot="footer" class="dialog-footer">
				...
				<el-button type="primary" class="admin-btn" @click="checkPurchase()">确 定</el-button>
			</span>
		</el-dialog>
		<!-- 结束增加弹出框 -->
		...
		<el-dialog title="用章审批表" :visible="editVisible2" width="950px">
			<div id="printTest2"  style="width:950px">
			<p style="width:90%;text-align:center;"><span style="width:100%;font-size:24px">{{editForm.planName}}</span></p>
			<br>
			<table id="table" border="0" cellspacing="0" style="width:90%">
				<thead>
					<tr>
						<td width="100%" style="text-align:center"><H2>保定市莲池卫健局用章审批表</H2></td>
					</tr>
				</thead>
			</table>
			<br>
			...
			<br>
				<table id="table" border="0" cellspacing="0" style="width:90%">
					<thead>
							<tr>
								<td width="100%" style="text-align:center">
									<button v-print="'#printTest2'">打印</button>
	 ...
	 methods: {
        ...
        handleEdit3(index, row) {
		 ...
		 checkPurchase() {
			var param={
                 action:"update",
				 model:"tb_purchase_record_recordfile",
				 recordFile1:this.recordFile1,
				 recordFile2:this.recordFile2,
		...
		if(response.code == 0){
			this.recordFile1 = response.data.fileurl;
		}
        },
        handleUploadSuccess2(response, file, fileList) {
		console.log('导入成功 response', response)
		if(response.code == 0){
			this.recordFile2 = response.data.fileurl;
		}
		},
	 },
};
