   ...
   <div class="wrapper-top">
    ...
    <el-row :gutter="20" >
          <el-col :span="24" style="text-align:center;width:100%;padding-top: 40px;padding-bottom: 20px;">
              <span class="system_title">预约凭证查询</span>
          </el-col>
    </el-row>
    <el-row :gutter="20" class="areacontent" >
          <el-col :span="24">
            <div class="searchtable">
                <el-row :gutter="20" >
                    <el-col :span="19"><input type="text" v-model="keyWord" class="search_input" placeholder="请输入身份证号码"/></el-col>
    ...
    <el-row :gutter="20" v-if="is_loading">
          <el-col :span="24">
               <div class="itemtable">
                ...
                <span style="font-size:14px">请正确输入您要查询的身份证号</span>
              </div>
          </el-col>
    </el-row>
    </div>
  ...
  methods: {
      ...
      getResult1() {
			 ...
			 axios.post({url: api.commn.action,
				 data: {idcard:this.keyWord,model:'tb_check_applybyId',action:'selectDetail'}
			 }).then(res => {
                ...
                if(null != this.result1)
                {
                    ...
                    //   if(this.result1.payType == '2')
                    //   {
                    //     if(this.result1.paystate == '0')
                    //     {

                    //       this.$confirm('此订单属于自费订单，但未缴费，不能完成采集，请缴费?', '提示', {
                    //               confirmButtonText: '确定',
                    //               cancelButtonText: '取消',
                    //               type: 'warning'
                    //             }).then(() => {
                    //                 let url = 'https://zy.gk0312.cn/#/wechat/applypay?paynumber='+this.result1.paynumber+'&orgnumber='+this.result1.checkOrgNumber;
                    //                 window.location.href = url;
                    //                 //
                    //             }).catch(() => {
                    //               //几点取消的提示
                    //               this.$router.push({ path: "/applysuccess?applyNumber="+this.result1.applyNumber});
                    //         });
                    //     }else
                    //     {
                    //        this.$router.push({ path: "/applysuccess?applyNumber="+this.result1.applyNumber});
