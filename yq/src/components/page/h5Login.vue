          ...
          <tr>
           <td style="width:100%;text-align:center">
            <div style="padding-top:120px">
               <span style="font-size: 18px;color:#ffffff;font-weight:bold;">保定市莲池区</span><br>
               <span style="font-size: 20px;color:#ffffff;font-weight:bold;">采购审批流程系统</span>
            </div>
           </td>
         </tr>
         <tr>
          ...
          <td>
                <div class="wrapper-item3">
                    <table class="table-style">
                        <tr>
                            <td style="text-align:center"><span class="title-address">保定市莲池区长城北大街2109号</span></td>
                        </tr>
                    </table>
                </div>
          </td>
...
export default {
      ...
      if(null != username){
        ...
        if(code){
            ...
            console.log('openid', this.openid)
			    ...
			    let url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx8be5d1df71697aca&redirect_uri='+ window.location.href.split('#')[0] + '?from=h5login&response_type=code&scope=snsapi_base#wechat_redirect';
  ...
  methods: {
          getOpenid(code) {
            ...
            return new Promise((resolve, reject) => {
                axios.get({
                    ...
                    console.log(api.wechat.getOpenid, ' success', res)
                    if(res.code == 0){
                        //alert('获取openid接口返回数据:'+ JSON.stringify(res.data))
          ...
          axios.post({
            ...
            .then(res => {
              if (res && res.code == 0) {
                ...
                this.updateOpenid(this.ruleForm.username);
    ...
    updateOpenid(userName)
    {
          ...
          if (res.code == 0) {
              alert('绑定成功');
             //this.$router.push({ path: "/wechat/noneorderlist?orgnumber"+this.orgnumber,});
          }
      });
    },
  }
