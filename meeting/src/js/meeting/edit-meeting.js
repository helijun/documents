    ...
    HSKJ.ready(function () {
        var editMeeting = {
            ...
            data: {
                ...
                backgroundimage: meetingData.backgroundimage,//默认表头
                uprighscreensaver: meetingData.uprighscreensaver,//立式机屏保
                uprightui: meetingData.uprightui, //立式机识别界面
                panelscreensaver: meetingData.panelscreensaver, //平板签到屏保
                meetingdatetime: meetingData.holdtimestart.substr(0, 16) + ' ~ ' + meetingData.holdtimeend.substr(0, 16), //举办时间范围 3
                meetingApplyTime: meetingData.applystarttime.substr(0, 16) + ' ~ ' + meetingData.applyendtime.substr(0, 16), //报名有效期 1
                meetingsigntime: meetingData.signinstarttime.substr(0, 16) + ' ~ ' + meetingData.signinendtime.substr(0, 16) //签到有限期 2
            },
            ...
            renderHtml: function () {
                ...
                HSKJ.renderTpl(
                    '.module-container',
                    'text!tpl/meeting/edit-meeting.tpl',
                    Object.assign(self.data, meetingData),
                function () {
            ...
            renderMeetingApplyTime: function (max) {
                ...
                layui.laydate.render({
                    ...
                    , value: self.data.meetingApplyTime
                    , done: function (value, date) {
            ...
            renderMeetingsigntime: function () {
                ...
                layui.laydate.render({
                    ...
                    , value: self.data.meetingsigntime
                    , format: 'yyyy-MM-dd HH:mm'
                    , done: function (value, date) {
                        $('.change-meeting-info-datetime').children('span').html(value)
                    }
            ...
            withChange: function(){
                ...
                layui.form.on('submit(element-save)', function (data) {
                    ...
                    var holdtime = {
                        ...
                        holdtimeend: data.field.meetingdatetime.split('~')[1].trim()+":00"
                    };
                ...
                layui.form.on('submit(element-save2)', function (data) {
                    ...
                    var applyTime = {
                        ...
                        applyendtime: data.field.meetingapplytime.split('~')[1].trim()+":00",
                    };
                ...
                layui.form.on('submit(element-complete)', function (data) {
                    ...
                    var signintime = {
                        ...
                        signinendtime: data.field.meetingsigntime.split('~')[1].trim()+":00",
                    };
                ...
                HSKJ.POST({
                    ...
                    success: function (json) {
                        if(json && json.code == 0){
                            ...
                            $('.element-success-tips').html('您的会议已修改成功! ');
